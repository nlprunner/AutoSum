...
def filter_dataset_seq(labelId, sentences, embedding_matrix):
    x = []
    max_score = 0
    return avg_embedding(sentences, embedding_matrix), embedding_matrix[labelId]
if __name__ == "__main__":
    ...
    for train, test in kf.split(y):
        ...
        x = keras.layers.concatenate([word_input, context_input], axis=-1)
        x = Dense(64, activation='relu')(x)
        x = Dense(64, activation='relu')(x)
        main_output = Dense(1, activation='sigmoid', name='main_output')(x)
        model = Model(inputs=[word_input, context_input], outputs=main_output)
